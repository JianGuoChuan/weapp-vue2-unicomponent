<template>
	<view class="ybh-modal__container" :class="[show ? 'ybh-modal-show' : '']" :style="{zIndex:zIndex}" @touchmove.stop.prevent>
		<view :style="{ width: width, padding: padding, borderRadius: radius, backgroundColor: backgroundColor,zIndex:zIndex+1 }"
			...
			<view v-if="!custom">
				...
				<view class="ybh-modal-content" :class="[title ? '' : 'ybh-mtop']" :style="{ color: color, fontSize: size + 'rpx' }">
					<view :class="[ scanInput ? 'margin-b-md' : '' ]">{{ content }}</view>
					<view v-if="scanInput" class="flex space-between">
						<InputBar v-model="value" :placeholder="placeholder" class='width-all'></InputBar>
						<view class="text-left padding-top-sm padding-left-sm">
							<text class="icon-ecrt icon-ecrt-scan font-bold margin-l-xs" style="font-size: 68rpx;"
								@tap='scan'></text>
						</view>
					</view>
				</view>
		...
		<view class="ybh-modal-mask" :class="[show ? 'ybh-mask-show' : '']" :style="{zIndex:maskZIndex}" @tap="clickMask"></view>
	</view>
</template>
...
export default {
	...
	* 组件名称：模态框
	* props：
	* show：显隐开关，需开启.sync使用
	* width: 模态框宽度，百分比
	* backgroundColor： 模态框背景颜色
	* padding: 内边距
	* radius: 模态框圆角值，不是按钮的圆角
	* title：标题
	* content： 内容文本
	* placeholder：输入模式input的占位符
	* color：content的文本颜色
	* size：content的文本大小,默认单位rpx，无需自己填写
	* shape：按钮形状，默认square， 可选值：circle，半圆角
	* button：自定义按钮组，配合多按钮需求，可以自行传入按钮组配置项，见下方示例
	* scanInput：扫码模式
	* maskClosable: 点击遮罩是否可以关闭
	* autoClose: 点击按钮是否自动关闭，默认true。关闭后需配合按钮回调自行关闭模态框
	* fadeIn: 是否启用淡入淡出
	* custom: 是否启用自定义内容，开启后配合插槽使用
	* zIndex：模态框层级
	* maskZIndex： 遮罩层级
	*/
   // 示例：
   // <Modalplus :show.sync='modalShow' width='100%' backgroundColor='#f0f0f0' padding='10px 10px' radius='25px'
   // 	title='警告' content='删除后不可恢复' placeholder='请输入密码' color='red' size='30' shape='square'
   // 	:scanInput='true' :fadeIn='false' :maskClosable='false' :autoClose='false'></Modalplus>
	name: 'ecrtModalPlus',
	emits: [ 'click', 'cancel' ],
	components: {InputBar},
	props: {
	...
	methods: {
		...
		async scan(){
			...
			// #endif
			// #ifdef APP-PLUS
			let res = await this.$etrcKit.scanCode();
